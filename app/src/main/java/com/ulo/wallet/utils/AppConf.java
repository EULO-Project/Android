...
public class AppConf extends Configurations {
    ...
    public void saveTrustedNode(UloNodeData uloNodeData){
        save(TRUSTED_NODE_HOST, uloNodeData.getHost());
        save(TRUSTED_NODE_TCP, uloNodeData.getTcpPort());
//        save(TRUSTED_NODE_SSL,uloNodeData.getSslPort());
    }
    public UloNodeData getTrustedNode(){
        String host = getString(TRUSTED_NODE_HOST,null);
        if (host!=null){
            int tcp = getInt(TRUSTED_NODE_TCP,-1);
//            int ssl = getInt(TRUSTED_NODE_TCP,-1);
            return new UloNodeData(host,tcp);
        }else
            return null;
    }
