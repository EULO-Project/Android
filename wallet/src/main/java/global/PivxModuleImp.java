...
public class PivxModuleImp implements UloModule {
...
//    }
    ...
    public PivxModuleImp(ContextWrapper contextWrapper, WalletConfiguration walletConfiguration, ContactsStoreDao contactsStore, RateDbDao rateDb) {
        this.context = contextWrapper;
        this.walletConfiguration = walletConfiguration;
        this.contactsStore = contactsStore;
        this.rateDb = rateDb;
//        this.backupHelper = backupHelper;
    ...
    @Override
    public void createWallet() throws WalletManager.WalletErrorException {
    ...
    public void savePinCode(String pinCode) {
        walletManager.savePinCode(pinCode);
    }

    @Override
    public void addNode(String ip, int port) throws WalletManager.WalletErrorException {
        walletManager.addNode(ip,port);
    }

    @Override
    public void upDatePinCode(String oldpinCode,String newpinCode) throws WalletManager.WalletErrorException {
        walletManager.updatePassword(oldpinCode,newpinCode);
    }

    @Override
    public boolean backupWallet()   {
        return walletManager.backUpWallet();
    }
    ...
    @Override
    public void restoreWallet(List<String> mnemonic) throws IOException, MnemonicException {
        walletManager.restoreWalletFrom(mnemonic);
    }
    ...
    @Override
    public Coin getALLBalanceCoin() {
        return walletManager.getALLBalance();
    }
    ...
    public void saveContact(AddressLabel addressLabel) throws ContactAlreadyExistException {
        if (contactsStore.getContact(addressLabel.getAddresses().get(0)) != null)
    ...
    public boolean chechAddress(String addressBase58) {
        return walletManager.checkAddress(addressBase58);
    }
...
//        return (blockchainManager != null && peers != null) && !peers.isEmpty();
        ...
        if (getConnectStatus() == wallet.Wallet.CONNECT_STATUS_CONNECTED) {
            return true;
        } else return false;
    }
    ...
    public long getConnectedPeerHeight() {
        long height = 0;
        try {
            height = walletManager.getBlockHeight();
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//            return -1;
    ...
    @Override
    public List<wallet.transaction.Transaction> listTx() {
//        List<TransactionWrapper> list = new ArrayList<>();
//        walletManager.listTransactions();
//        for (wallet.transaction.Transaction transaction : walletManager.listTransactions()) {
...
//        }
        ...
        lastTransactionLength=list.size();
        return list;
    }
    int lastTransactionLength=0;
    List<wallet.transaction.Transaction> list;
    @Override
    public boolean checkTransactionChanged()  {
        try {
            if(isSyncWithNode()){
                list=walletManager.listTransactions();
                int nowLength=list.size();
                if(lastTransactionLength!=nowLength){
                    return true;
                }
            }
...
//    }
    ...
    public long getChainHeight() {
        return walletManager.getLastBlockHeight();
    }
...
//        return context.getRate();
    ...
    public boolean isSyncWithNode() throws NoPeerConnectedException {
        ...
        if (isAnyPeerConnected()) {
            ...
            isSync = walletManager.ifsSyncFinished();
...
//    }
    ...
    public long getConnectStatus() {
        return walletManager.getConnectStatus();
    }

    @Override
    public String createTransaction(long amount, String addr, String des) {
        return walletManager.createTransaction(amount, addr, des);
    }
    ...
    @Override
    public void resetBlockChain() throws WalletManager.WalletErrorException {
